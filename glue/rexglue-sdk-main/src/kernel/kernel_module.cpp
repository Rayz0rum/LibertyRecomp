...
uint32_t KernelModule::GetProcAddressByOrdinal(uint16_t ordinal) {
  ...
  if (!export_entry) {
    REXKRNL_DEBUG("GetProcAddressByOrdinal: ordinal {:04X} not found in {}", ordinal, name_);
    return 0;
  }

  if (export_entry->type == rex::runtime::Export::Type::kVariable) {
    // Variables have guest addresses we can return directly
    REXKRNL_DEBUG("GetProcAddressByOrdinal: {} ({:04X}) -> variable at {:08X}",
                  export_entry->name, ordinal, export_entry->variable_ptr);
  ...
  REXKRNL_WARN("GetProcAddressByOrdinal: function {} ({:04X}) in {} - no guest thunk available",
...
uint32_t KernelModule::GetProcAddressByName(const std::string_view name) {
  // TODO: Does this even work for kernel modules?
    (void)name;
  REXKRNL_ERROR("KernelModule::GetProcAddressByName not implemented");
  return 0;
}

...
X_STATUS UserModule::LoadFromFile(const std::string_view path) {
  fprintf(stderr, "[TRACE] UserModule::LoadFromFile enter - %.*s\n", (int)path.size(), path.data()); fflush(stderr);
  ...
  if (fs_entry->can_map()) {
    ...
    // Load the module.
    fprintf(stderr, "[TRACE] LoadFromFile - calling LoadFromMemory (mmap, size=%zu)\n", mmap->size()); fflush(stderr);
    result = LoadFromMemory(mmap->data(), mmap->size());
    fprintf(stderr, "[TRACE] LoadFromFile - LoadFromMemory returned %08X\n", result); fflush(stderr);
  } else {
  ...
  fprintf(stderr, "[TRACE] LoadFromFile - calling LoadXexContinue\n"); fflush(stderr);
...
X_STATUS UserModule::LoadXexContinue() {
  fprintf(stderr, "[TRACE] UserModule::LoadXexContinue enter\n"); fflush(stderr);
  ...
  fprintf(stderr, "[TRACE] LoadXexContinue - calling xex_module()->LoadContinue()\n"); fflush(stderr);
  if (!xex_module()->LoadContinue()) {
    return X_STATUS_UNSUCCESSFUL;
  }
  fprintf(stderr, "[TRACE] LoadXexContinue - LoadContinue() done\n"); fflush(stderr);

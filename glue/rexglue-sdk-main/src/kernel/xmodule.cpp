...
bool XModule::Matches(const std::string_view name) const {
  return rex::string::utf8_equal_case(rex::string::utf8_find_name_from_guest_path(path()),
                              name) ||
...
bool XModule::Save(stream::ByteStream* stream) {
  REXKRNL_DEBUG("XModule {:08X} ({})", handle(), path());
...
object_ref<XModule> XModule::Restore(KernelState* kernel_state,
  ...
  REXKRNL_DEBUG("XModule {:08X} ({})", module->handle(), module->path());

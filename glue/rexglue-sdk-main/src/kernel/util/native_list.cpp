...
namespace rex::kernel::util {

NativeList::NativeList() = default;

NativeList::NativeList(memory::Memory* memory)
...
bool NativeList::IsQueued(uint32_t ptr) {
  uint32_t flink =
      memory::load_and_swap<uint32_t>(memory_->TranslateVirtual(ptr + 0));
  uint32_t blink =
      memory::load_and_swap<uint32_t>(memory_->TranslateVirtual(ptr + 4));

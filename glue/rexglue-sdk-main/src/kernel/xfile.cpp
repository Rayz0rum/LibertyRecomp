...
X_STATUS XFile::Read(uint32_t buffer_guest_address, uint32_t buffer_length,
  ...
  if (buffer_length) {
    if (UINT32_MAX - buffer_guest_address < buffer_length) {
      ...
      rex::memory::BaseHeap* buffer_start_heap =
...
X_STATUS XFile::ReadScatter(uint32_t segments_guest_address, uint32_t length,
  ...
  rex::be<uint32_t>* segments = reinterpret_cast<rex::be<uint32_t>*>(
...
bool XFile::Save(stream::ByteStream* stream) {
  REXKRNL_DEBUG("XFile {:08X} ({})", handle(),
  ...
  stream->Write<bool>(
      (file_->entry()->attributes() & rex::filesystem::kFileAttributeDirectory) != 0);
...
object_ref<XFile> XFile::Restore(KernelState* kernel_state,
  ...
  REXKRNL_DEBUG("XFile {:08X} ({})", file->handle(), abs_path);

  rex::filesystem::File* vfs_file = nullptr;
  rex::filesystem::FileAction action;
  auto res = kernel_state->file_system()->OpenFile(
      nullptr, abs_path, rex::filesystem::FileDisposition::kOpen, access, is_directory,
      false, &vfs_file, &action);
  if (XFAILED(res)) {
    REXKRNL_ERROR("Failed to open XFile: error {:08X}", res);

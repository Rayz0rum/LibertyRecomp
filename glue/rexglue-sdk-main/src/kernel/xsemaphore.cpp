...
bool XSemaphore::InitializeNative(void* native_ptr, X_DISPATCH_HEADER* header) {
  ...
  semaphore_ = rex::thread::Semaphore::Create(semaphore->header.signal_state,
                                                semaphore->limit);
  return !!semaphore_;
}
...
bool XSemaphore::Save(stream::ByteStream* stream) {
  ...
  REXKRNL_DEBUG("XSemaphore {:08X} (count {}/{})", handle(), free_count,
         maximum_count_);
...
object_ref<XSemaphore> XSemaphore::Restore(KernelState* kernel_state,
  ...
  REXKRNL_DEBUG("XSemaphore {:08X} (count {}/{})", sem->handle(), free_count,
         sem->maximum_count_);

  sem->semaphore_ =
      rex::thread::Semaphore::Create(free_count, sem->maximum_count_);

...
dword_result_t NtQueryInformationFile_entry(
  ...
  switch (info_class) {
    ...
    case XFileSectorInformation: {
      // Host filesystem has no DVD sectors.  Return sector 0 so the game's
      // RPF streaming code gets a valid base offset instead of using
      // uninitialised data (which corrupts the heap).

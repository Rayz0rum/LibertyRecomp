...
struct alignas(O1HEAP_ALIGNMENT) SizeHeader {
  uint64_t requested_size;
  uint8_t _pad[O1HEAP_ALIGNMENT - sizeof(uint64_t)];
};
...
uint32_t ReXHeap::Realloc(uint32_t guest_addr, uint32_t new_size, bool zero_new) {
  ...
  // o1heap doesn't have realloc — alloc new, copy, free old
  void* new_ptr = o1heapAllocate(heap_, new_size + kHeaderSize);
  if (!new_ptr) {
    REXKRNL_WARN("rexcrt_RtlReAllocateHeap: o1heapAllocate({}) failed", new_size);
    return 0;
  }
  ...
  void* new_user = static_cast<uint8_t*>(new_ptr) + kHeaderSize;
  void* old_user = static_cast<uint8_t*>(real_host) + kHeaderSize;
  uint32_t copy_size = old_size < new_size ? old_size : new_size;
  std::memcpy(new_user, old_user, copy_size);

  if (zero_new && new_size > old_size) {
    std::memset(static_cast<uint8_t*>(new_user) + old_size, 0, new_size - old_size);
  }

  o1heapFree(heap_, real_host);

  return HostToGuest(new_user);
}

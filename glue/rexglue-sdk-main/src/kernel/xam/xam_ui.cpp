...
dword_result_t XamShowMessageBoxUI_entry(
  ...
  // Auto-pick the focused button (headless) or show dialog.
  auto run_headless = [result_ptr, active_button]() -> X_RESULT {
    *result_ptr = static_cast<uint32_t>(active_button);
    return X_ERROR_SUCCESS;
  };

#if defined(REX_HEADLESS)
  X_RESULT result = xeXamDispatchHeadless(run_headless, overlapped.guest_address());
#else
  X_RESULT result;
  if (REXCVAR_GET(headless)) {
    result = xeXamDispatchHeadless(run_headless, overlapped.guest_address());
  } else {
...
dword_result_t XamShowKeyboardUI_entry(
  ...
  auto run_headless = [default_text, buffer, buffer_length,

...
SigScanner::SigScanner(const runtime::Module& module)
    : module_(module)
{
}
...
std::vector<uint32_t> SigScanner::scan(const Signature& sig) {
    ...
    for (const auto& section : module_.binary_sections()) {
        if (!section.host_data) continue;

        if (!section.executable) continue;

        auto rangeMatches = scanRange(
...
SigScanner::scanAll(const std::vector<Signature>& sigs) {
...
std::vector<uint32_t> SigScanner::scanRange(
    ...
    size_t entryOffset)
{
    ...
    for (uint32_t addr = start; addr + patternBytes <= scanEnd; addr += 4) {
        ...
        if (matched) {
            ...
            REXCODEGEN_TRACE("SigScanner: pattern match at 0x{:08X}, entry=0x{:08X}",
                            addr, entryPoint);
        }
    }

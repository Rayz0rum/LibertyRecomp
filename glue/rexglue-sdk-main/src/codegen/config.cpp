...
void RecompilerConfig::Load(const std::string_view& configFilePath)
{
    ...
    if (auto functionsTable = toml["functions"].as_table())
    {
        for (auto& [key, value] : *functionsTable)
        {
            ...
            if (cfg.end && cfg.end <= address) {
                REXCODEGEN_ERROR("Function 0x{:08X}: 'end' (0x{:08X}) must be greater than address", address, cfg.end);
                continue;
            }
        ...
        if (!functions.empty()) {
            ...
            for (const auto& [addr, cfg] : functions) {
                if (cfg.isChunk()) chunks_count++;
            }
            REXCODEGEN_INFO("Loaded {} function configs ({} standalone, {} chunks)",
                           functions.size(), functions.size() - chunks_count, chunks_count);
        }
    }
    ...
    if (auto switchTableArray = toml["switch_tables"].as_array())
    {
        for (auto& entry : *switchTableArray)
        {
            ...
            REXCODEGEN_DEBUG("Loaded manual switch table at 0x{:08X} with {} labels", *address_opt, label_count);
        }
    }
            ...
            if (it != seen.end()) {
                if (it->second == funcSize) {
                    result.errors.push_back(fmt::format(
                        "Duplicate function boundary: 0x{:08X} size 0x{:X}", addr, funcSize));
                } else {
                    result.errors.push_back(fmt::format(
                        "Conflicting sizes at 0x{:08X}: 0x{:X} vs 0x{:X}",
    ...
    // Check for overlapping function boundaries (standalone functions only)
    {
        ...
        for (size_t i = 1; i < sorted.size(); ++i) {
            ...
            if (sorted[i].first < prev_end) {
                result.errors.push_back(fmt::format(
                    "Overlapping boundaries: 0x{:08X}+0x{:X} overlaps 0x{:08X}+0x{:X}",
                    sorted[i-1].first, sorted[i-1].second,

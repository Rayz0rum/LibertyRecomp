...
BlockDiscoveryResult discoverBlocks(
    ...
    // Safety cap: no single function should exceed 1 MB of code.
    // Jump table targets can extend funcEnd, but never past this absolute limit.
    constexpr uint32_t kMaxFunctionSize = 1024 * 1024;
    const uint32_t funcEndCap = std::min(containingRegion.end, entryPoint + kMaxFunctionSize);
                ...
                if (isCall(*insn)) {
                    ...
                    if (jt) {
                        ...
                        for (uint32_t t : jt->targets) {
                            if (t >= funcEnd && t < funcEndCap) {
                                funcEnd = t + 4;  // Extend to include this target
                            } else if (t >= funcEndCap) {
                                REXCODEGEN_WARN("discoverBlocks: 0x{:08X} jump table target 0x{:08X} exceeds function size cap, ignoring",
                                               entryPoint, t);
                            }

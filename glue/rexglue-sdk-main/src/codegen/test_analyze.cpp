    ...
    size_t dataSize)
{
    ...
    for (size_t i = 0; i < testFunctions.size(); i++) {
        ...
        auto* node = ctx.graph.addFunction(
            fnAddr,
            fnSize,
            FunctionAuthority::DISCOVERED,
            true
        );

        if (node) {
            ctx.graph.addBlockToFunction(fnAddr, {fnAddr, fnSize});
            ctx.graph.setFunctionName(
                fnAddr,
                fmt::format("{}_{:X}", testName, fnAddr)
            );
        }
    }
    ...
    for (size_t i = 0; i < testFunctions.size(); i++) {
        ...
        for (uint32_t offset = 0; offset < fnSize; offset += 4) {
            ...
            uint32_t raw = load_and_swap<uint32_t>(
                data + (fnAddr - baseAddress) + offset);
            ...
            if (decoded.is_call() && decoded.branch_target.has_value()) {
                ctx.graph.addUnresolvedJumpToFunction(
                    fnAddr, pc, decoded.branch_target.value(), true, false);
            }
        }
    }
}

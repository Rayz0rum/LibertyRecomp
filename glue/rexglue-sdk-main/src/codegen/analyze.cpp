...
void discoverAllFunctions(CodegenContext& ctx) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        REXCODEGEN_INFO("Analyze: discovery iteration {} — {} total functions, {} to discover",
                       iteration, currentFunctionCount, needsDiscovery.size());
        REX_LOG_FLUSH();
    ...
    // VTable scanning
    {
        ...
        if (newFunctions > 0) {
            ...
            while (vtableIteration < MAX_VTABLE_ITERATIONS) {
                ...
                REXCODEGEN_DEBUG("Analyze: vtable discovery iteration {} ({} functions)",
                                vtableIteration, graph.functionCount());
...
void cleanupAbsorbedGapFills(CodegenContext& ctx) {
    ...
    // Build sorted list of non-GAP_FILL function intervals [base, base+size)
    // so we can binary-search instead of O(N²) nested iteration.
    struct Interval {
        uint32_t start;
        uint32_t end; // exclusive
    };
    std::vector<Interval> nonGapIntervals;
    nonGapIntervals.reserve(graph.functionCount());

    for (const auto& [addr, node] : graph.functions()) {
        if (node->authority() == FunctionAuthority::GAP_FILL) continue;
        if (node->size() == 0) continue;
        nonGapIntervals.push_back({addr, addr + node->size()});
    }
    std::sort(nonGapIntervals.begin(), nonGapIntervals.end(),
              [](const Interval& a, const Interval& b) { return a.start < b.start; });

    // Also collect GAP_FILL addresses sorted, so lower-address GAP_FILLs
    // absorb higher-address ones in a single pass.
    std::vector<std::pair<uint32_t, uint32_t>> gapFills; // (addr, end)
    for (const auto& [addr, node] : graph.functions()) {
        if (node->authority() != FunctionAuthority::GAP_FILL) continue;
        gapFills.emplace_back(addr, addr + node->size());
    }
    std::sort(gapFills.begin(), gapFills.end());

    REXCODEGEN_INFO("Analyze: cleanupAbsorbedGapFills checking {} GAP_FILL against {} other functions",
                   gapFills.size(), nonGapIntervals.size());
    REX_LOG_FLUSH();

    std::vector<uint32_t> toRemove;

    for (const auto& [gapAddr, gapEnd] : gapFills) {
        // Binary search: find the last interval whose start <= gapAddr
        auto it = std::upper_bound(nonGapIntervals.begin(), nonGapIntervals.end(), gapAddr,
            [](uint32_t addr, const Interval& iv) { return addr < iv.start; });

        // Check the interval just before upper_bound (it starts <= gapAddr)
        if (it != nonGapIntervals.begin()) {
            --it;
            if (gapAddr >= it->start && gapAddr < it->end) {
                toRemove.push_back(gapAddr);
                continue;
            }
        }
    }

    // Second pass: among remaining GAP_FILLs, lower-address absorbs higher
    // Build a simple sorted sweep: if a GAP_FILL's address falls inside an
    // earlier GAP_FILL's range, remove it.
    std::unordered_set<uint32_t> removedSet(toRemove.begin(), toRemove.end());
    uint32_t coverEnd = 0;
    for (const auto& [gapAddr, gapEnd] : gapFills) {
        if (removedSet.count(gapAddr)) continue;
        if (gapAddr < coverEnd) {
            toRemove.push_back(gapAddr);
            continue;
        }
...
void mergeAndSeal(CodegenContext& ctx) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        REXCODEGEN_DEBUG("Analyze: mergeAndSeal iteration {} — {} pending functions",
                        iteration, pendingAddrs.size());
...
VoidResult validateGraph(CodegenContext& ctx) {
    ...
    if (errors.HasErrors()) {
        auto unresolved = errors.Count(AnalysisErrors::Category::UnresolvedCall);
        REXCODEGEN_WARN("Analyze: found {} validation issues ({} unresolved calls)",
                        errors.Count(), unresolved);
        errors.PrintReport();
        // Treat unresolved calls as warnings — a small number of unresolved
        // tail-call targets in a large binary is expected and non-fatal.
        // The recompiler will emit runtime stubs for these.
        REXCODEGEN_WARN("Analyze: proceeding despite {} unresolved calls", unresolved);
    } else {
...
Result<void> Analyze(CodegenContext& ctx) {
    ...
    auto phase_start = pipeline_start;

    auto logPhase = [&](const char* name) {
        auto now = std::chrono::steady_clock::now();
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - phase_start).count();
        fprintf(stderr, "[TRACE] Analyze: [%s] completed in %lld ms\n", name, (long long)ms); fflush(stderr);
        REXCODEGEN_INFO("Analyze: [{}] completed in {} ms", name, ms);
        REX_LOG_FLUSH();
        phase_start = now;
    };

    fprintf(stderr, "[TRACE] Analyze: calling initDecoded()...\n"); fflush(stderr);
    ctx.initDecoded();
    fprintf(stderr, "[TRACE] Analyze: initDecoded() done\n"); fflush(stderr);
    ...
    fprintf(stderr, "[TRACE] Analyze: gapFillCodeRegions...\n"); fflush(stderr);
    // 4. Gap fill uncovered regions
    gapFillCodeRegions(ctx);
    logPhase("gapFillCodeRegions");
    ...
    logPhase("gapFillDiscovery");

    fprintf(stderr, "[TRACE] Analyze: cleanupAbsorbedGapFills...\n"); fflush(stderr);
    // 5.5. Remove absorbed GAP_FILL functions
    cleanupAbsorbedGapFills(ctx);
    logPhase("cleanupAbsorbedGapFills");

    fprintf(stderr, "[TRACE] Analyze: mergeAndSeal...\n"); fflush(stderr);
    // 6. Merge: resolve jumps and seal functions
    mergeAndSeal(ctx);
    logPhase("mergeAndSeal");

    fprintf(stderr, "[TRACE] Analyze: validateGraph...\n"); fflush(stderr);
    // 7. Validate
    auto validateResult = validateGraph(ctx);
    if (!validateResult) {
        return validateResult;
    }
    logPhase("validateGraph");

    auto total_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - pipeline_start).count();
    REXCODEGEN_INFO("Analyze: complete - {} functions ready for code generation (total {} ms)",
                   ctx.graph.functionCount(), total_ms);
    REX_LOG_FLUSH();

...
DecodedBinary::DecodedBinary(const BinaryView& binary)
    : binary_(binary) {
}
...
const uint8_t* DecodedBinary::rawData(uint32_t addr, size_t len) const {
    ...
    if (offset + len > sec->data.size()) return nullptr;
...
DecodedBinary::Section* DecodedBinary::findSection(uint32_t addr) {
    for (auto& sec : sections_) {
        if (sec.contains(addr)) return &sec;
    }
    return nullptr;
}
...
void DecodedBinary::computeCodeRegions() {
    ...
    for (const auto& sec : sections_) {
        if (sec.instructions.empty()) continue;

...
VTableScanner::VTableScanner(const BinaryView& binary)
    : binary_(binary)
{
}
...
std::vector<VTableInfo> VTableScanner::scan() {
    ...
    for (uint32_t colAddr : cols) {
        ...
        REXCODEGEN_DEBUG("VTableScanner: vtable at 0x{:08X} ({}) has {} slots",
                        info.vtableAddress, info.className, info.slots.size());

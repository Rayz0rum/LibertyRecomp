...
BinaryView BinaryView::fromModule(const runtime::Module& module) {
    ...
    for (const auto& sym : module.binary_symbols()) {
        if (sym.type == runtime::BinarySymbolType::Import) {
            ...
            view.importSymbols_.push_back(ImportSymbol{
                .address = sym.address,
                .name = sym.name
            });
    ...
    for (const auto& section : binarySections) {
        ...
        // Copy bytes
        view.sectionData_.emplace_back(
            section.host_data,
            section.host_data + section.virtual_size
        );

        // Create view pointing into our owned data
        view.sections_.push_back(SectionView{
        ...
        REXCODEGEN_DEBUG("BinaryView: section '{}' at 0x{:08X} size 0x{:X} exec={}",
                        section.name, section.virtual_address, section.virtual_size, section.executable);
    }

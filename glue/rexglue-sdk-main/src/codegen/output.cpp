 ...
 */
 ...
 // 
 // The function graph should be used only for resolving imports, and all other data should be passed in directly.
...
void RecompilerOutput::add_jump_table(guest_addr_t func_address,
...
bool RecompilerOutput::write_all(const std::filesystem::path& output_dir) {
    ...
    if (!write_config_h(output_dir)) return false;
    if (!write_shared_h(output_dir)) return false;
    if (!write_recomp_files(output_dir)) return false;
    if (!write_init(output_dir)) return false;
    if (config_.emit_cmake && !write_cmake(output_dir)) return false;
...
bool RecompilerOutput::write_recomp_files(const std::filesystem::path& dir) {
    ...
    size_t num_files = (func_list.size() + config_.functions_per_file - 1) /
                       config_.functions_per_file;
...
bool RecompilerOutput::write_init(const std::filesystem::path& dir) {
    ...
    out << "#include <rex/runtime/processor.h>\n";
    out << "#include <cstdio>\n";
    out << "#include <cstdlib>\n\n";

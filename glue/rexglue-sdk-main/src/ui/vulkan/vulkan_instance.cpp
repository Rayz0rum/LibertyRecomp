...
std::unique_ptr<VulkanInstance> VulkanInstance::Create(
  ...
  // On macOS, MoltenVK provides the Vulkan ICD.  The Vulkan SDK installs a
  // loader as libvulkan.1.dylib; MoltenVK itself is libMoltenVK.dylib.
  // Try the SDK loader first, then fall back to MoltenVK directly.
  const char* const loader_library_name = "libvulkan.1.dylib";
  const char* const loader_library_name_fallback = "libMoltenVK.dylib";
#else
  ...
  if (!vulkan_instance->loader_) {
    vulkan_instance->loader_ =
        dlopen(loader_library_name_fallback, RTLD_NOW | RTLD_LOCAL);
  }
#endif
  if (!vulkan_instance->loader_) {

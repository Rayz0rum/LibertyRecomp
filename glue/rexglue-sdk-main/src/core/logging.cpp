...
REXCVAR_DEFINE_STRING(log_level, "info", "Log", "Global log level: trace, debug, info, warn, error, critical, off")
...
namespace {
    ...
    std::array<std::shared_ptr<spdlog::logger>,
               static_cast<size_t>(LogCategory::Count)> g_loggers;
...
std::optional<LogCategory> CategoryFromName(const std::string& name) {
    ...
    static const std::unordered_map<std::string, LogCategory> name_map = {
        ...
        {"apu", LogCategory::APU},
        {"audio", LogCategory::APU},      // Alias
        {"gpu", LogCategory::GPU},
        {"graphics", LogCategory::GPU},   // Alias
        {"kernel", LogCategory::Kernel},
        {"krnl", LogCategory::Kernel},    // Alias
        {"runtime", LogCategory::Kernel}, // Alias (maps legacy category)
        {"fs", LogCategory::FS},
        {"filesystem", LogCategory::FS},  // Alias

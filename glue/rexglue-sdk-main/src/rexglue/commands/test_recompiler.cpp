...
#include <rex/runtime/module.h>  // For Module interface
#include <rex/runtime/map_parser.h>
        ...
        if (!line.empty() && line[0] != '#') {
            ...
            if (colonIndex != std::string::npos) {
                ...
                if (!spec.inputs.empty() || !spec.outputs.empty() ||
                    !spec.mem_inputs.empty() || !spec.mem_outputs.empty()) {
...
bool recompile_tests(
    ...
    for (const auto& entry : fs::directory_iterator(binDirPath)) {
        ...
        auto ctx = codegen::CodegenContext::Create(
            codegen::BinaryView::fromModule(module),
            std::move(config));
        recompiler.ctx_ = &ctx;

        // Analyze functions using test_ prefixed symbols from map
        codegen::AnalyzeTestBinary(ctx, stem, symbols, TEST_BASE_ADDRESS, fileData.data(), fileData.size());
    ...
    // Write functions file with header and forward declarations
    {
        ...
        out << "#include <rex/runtime/guest.h>\n";
        out << "#include <rex/logging.h>  // For REX_FATAL on unresolved calls\n";
        out << "using namespace rex::runtime::guest;\n\n";
    ...
    testsOut << "#include <rex/runtime/guest.h>\n";
    testsOut << "#include <rex/kernel/xmemory.h>\n";
    ...
    testsOut << "        if (heap) {\n";
    testsOut << "            // Allocate from end of guard page (0x10000) up to cover test addresses\n";
    testsOut << "            heap->AllocFixed(0x10000, 0x10000000, 0x1000,\n";
    testsOut << "                rex::memory::kMemoryAllocationReserve | rex::memory::kMemoryAllocationCommit,\n";
    testsOut << "                rex::memory::kMemoryProtectRead | rex::memory::kMemoryProtectWrite);\n";
    testsOut << "        }\n";
    ...
    declsOut << "#include <rex/runtime/guest.h>\n\n";

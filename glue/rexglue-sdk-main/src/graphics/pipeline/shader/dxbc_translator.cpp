...
REXCVAR_DEFINE_BOOL(dxbc_switch, true,
    "Use switch statements in DXBC",
    "GPU/Shader");

REXCVAR_DEFINE_BOOL(dxbc_source_map, false,
    "Generate source maps for DXBC",
    "GPU/Shader");
...
void DxbcShaderTranslator::StartVertexOrDomainShader() {
  ...
  uint32_t interpolator_count =
      rex::bit_count(GetModificationInterpolatorMask());
...
void DxbcShaderTranslator::StartTranslation() {
  ...
  uint32_t interpolator_register_count =
      rex::bit_count(interpolator_register_mask);
  if (is_vertex_shader()) {
...
void DxbcShaderTranslator::WriteResourceDefinition() {
  ...
  if (cbuffer_index_bool_loop_constants_ != kBindingIndexUnallocated) {
    ...
    for (size_t i = 0; i < rex::countof(constant_register_map.bool_bitmap);
         ++i) {
                                                       ...
                                                       cbuffer_count_);
  {
    ...
    for (uint32_t i = 0; i < cbuffer_count_; ++i) {
      ...
      if (i == cbuffer_index_system_constants_) {
        ...
        cbuffer.size_vector_aligned_bytes =
            sizeof(uint32_t) *
            rex::align(GetBindlessResourceCount(), uint32_t(4));
      } else {
  ...
  if (is_vertex_shader()) {
    ...
    if (!edram_rov_used_) {
      ...
      uint32_t color_targets_written_count =
          rex::bit_count(color_targets_written);

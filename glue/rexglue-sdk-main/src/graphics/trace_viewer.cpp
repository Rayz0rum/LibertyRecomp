...
TraceViewer::TraceViewer(rex::ui::WindowedAppContext& app_context,
                         const std::string_view name)
    : rex::ui::WindowedApp(app_context, name, "some.trace"),
...
bool TraceViewer::OnInitialize() {
  ...
  if (path.empty()) {
    rex::ShowSimpleMessageBox(rex::SimpleMessageBoxType::Warning,
...
bool TraceViewer::Setup() {
  ...
  window_ = rex::ui::Window::Create(app_context(), "xenia-gpu-trace-viewer",
                                   1920, 1080);
  ...
  imgui_drawer_ =
      std::make_unique<rex::ui::ImGuiDrawer>(window_.get(), kZOrderImGui);
              ...
              switch (action.type) {
                case PacketAction::Type::kRegisterWrite: {
                  ...
                  if (!register_info ||
                    ...
                    ImGui::Text("%8f", rex::memory::Reinterpret<float>(
...
#define LOADEL(type, wo)                                                   \
  GpuSwap(memory::load<type>(vstart +                                          \
    ...
    if (draw_info.is_auto_index) {
      ...
      for (int i = display_start; i < display_end;
        ...
        uint32_t value = element_size == 4
                             ? GpuSwap(memory::load<uint32_t>(data_ptr),
                                       draw_info.index_endianness)
                             : GpuSwap(memory::load<uint16_t>(data_ptr),
                                       draw_info.index_endianness);
        ImGui::Text(" %d", value);
        ImGui::NextColumn();
      }

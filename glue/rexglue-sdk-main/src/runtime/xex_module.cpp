...
int XexModule::ReadImage(const void* xex_addr, size_t xex_length,
                         bool use_dev_key) {
  fprintf(stderr, "[TRACE] ReadImage enter - dev_key=%d\n", use_dev_key); fflush(stderr);
  ...
  int result_code = 0;
  fprintf(stderr, "[TRACE] ReadImage - compression_type=%d\n", opt_file_format_info()->compression_type); fflush(stderr);
  switch (opt_file_format_info()->compression_type) {
    case XEX_COMPRESSION_NONE:
      fprintf(stderr, "[TRACE] ReadImage - uncompressed\n"); fflush(stderr);
      result_code = ReadImageUncompressed(xex_addr, xex_length);
      break;
    case XEX_COMPRESSION_BASIC:
      fprintf(stderr, "[TRACE] ReadImage - basic compressed\n"); fflush(stderr);
      result_code = ReadImageBasicCompressed(xex_addr, xex_length);
      break;
    case XEX_COMPRESSION_NORMAL:
      fprintf(stderr, "[TRACE] ReadImage - normal compressed\n"); fflush(stderr);
...
int XexModule::ReadImageCompressed(const void* xex_addr, size_t xex_length) {
  fprintf(stderr, "[TRACE] ReadImageCompressed enter\n"); fflush(stderr);
  const uint32_t exe_length =
      static_cast<uint32_t>(xex_length - xex_header()->header_size);
  ...
  switch (opt_file_format_info()->encryption_type) {
    case XEX_ENCRYPTION_NONE:
      fprintf(stderr, "[TRACE] ReadImageCompressed - no encryption\n"); fflush(stderr);
      break;
    case XEX_ENCRYPTION_NORMAL:
      fprintf(stderr, "[TRACE] ReadImageCompressed - decrypting %u bytes...\n", exe_length); fflush(stderr);
      free_input = true;
      ...
      fprintf(stderr, "[TRACE] ReadImageCompressed - decryption done\n"); fflush(stderr);
      break;
    default:
      assert_always();
      return 1;
  ...
  while (cur_block->block_size) {
    ...
    while (true) {
      // Safety: ensure we don't read past the current block
      if (p + 2 > pnext) {
        REXLOG_ERROR("ReadImageCompressed: chunk header overrun in deblocking loop");
        result_code = 4;
        break;
      }
      ...
      // Safety: ensure chunk data doesn't exceed block or output buffer bounds
      if (p + chunk_size > input_buffer + input_size ||
          d + chunk_size > compress_buffer + exe_length) {
        REXLOG_ERROR("ReadImageCompressed: chunk data overrun (chunk_size={})", chunk_size);
        result_code = 4;
        break;
      }
  ...
  fprintf(stderr, "[TRACE] ReadImageCompressed - deblocking done, result_code=%d\n", result_code); fflush(stderr);
  if (!result_code) {
    uint32_t uncompressed_size = image_size();
    fprintf(stderr, "[TRACE] ReadImageCompressed - decompressing %u bytes (lzx)...\n", uncompressed_size); fflush(stderr);
    ...
    if (alloc_result) {
      ...
      fprintf(stderr, "[TRACE] ReadImageCompressed - lzx_decompress result=%d\n", result_code); fflush(stderr);
    } else {
      REXLOG_ERROR("Unable to allocate XEX memory at {:08X}-{:08X}.", base_address_,
             uncompressed_size);
      result_code = 3;
...
bool XexModule::Load(const std::string_view name, const std::string_view path,
  ...
  fprintf(stderr, "[TRACE] XexModule::Load enter - name=%.*s len=%zu\n", (int)name.size(), name.data(), xex_length); fflush(stderr);
  ...
  fprintf(stderr, "[TRACE] XexModule::Load - calling ReadImage (base=0x%08X)\n", base_address_); fflush(stderr);
  ...
  fprintf(stderr, "[TRACE] XexModule::Load - done\n"); fflush(stderr);
  return true;
}

bool XexModule::LoadContinue() {
  fprintf(stderr, "[TRACE] XexModule::LoadContinue enter\n"); fflush(stderr);
  ...
  fprintf(stderr, "[TRACE] LoadContinue - reading PE headers\n"); fflush(stderr);
  if (ReadPEHeaders()) {
    REXLOG_ERROR("Failed to load XEX PE headers!");
    return false;
  }
  fprintf(stderr, "[TRACE] LoadContinue - PE headers OK\n"); fflush(stderr);
  ...
  if (opt_import_libraries) {
    ...
    fprintf(stderr, "[TRACE] LoadContinue - processing imports (total_size=%u)...\n", (unsigned)opt_import_libraries->size); fflush(stderr);
    int lib_idx = 0;
    while (library_offset < opt_import_libraries->size) {
      auto library =
          reinterpret_cast<xex2_import_library*>(library_data + library_offset);
      fprintf(stderr, "[TRACE] LoadContinue - lib[%d] offset=%u size=%u count=%u\n",
              lib_idx, library_offset, (unsigned)library->size, (unsigned)library->count); fflush(stderr);
      if (!library->size) {
        break;
      }
      ...
      fprintf(stderr, "[TRACE] LoadContinue - importing '%s' (%u imports)\n",
              library_name.c_str(), (unsigned)library->count); fflush(stderr);
      SetupLibraryImports(library_name, library);
      fprintf(stderr, "[TRACE] LoadContinue - lib '%s' done\n", library_name.c_str()); fflush(stderr);
      library_offset += library->size;
      lib_idx++;
    }
  }

  fprintf(stderr, "[TRACE] LoadContinue - imports done, setting up memory protection...\n"); fflush(stderr);
  ...
  fprintf(stderr, "[TRACE] LoadContinue - populating binary data...\n"); fflush(stderr);
  // Populate binary introspection data
  PopulateBinaryData();

  fprintf(stderr, "[TRACE] LoadContinue - done\n"); fflush(stderr);
  return true;
}
...
bool XexModule::SetupLibraryImports(const std::string_view name,
  ...
  fprintf(stderr, "[TRACE] SetupLibraryImports enter - '%.*s' count=%u\n",
          (int)name.size(), name.data(), (unsigned)library->count); fflush(stderr);
  ...
  uint32_t import_count = library->count;
  fprintf(stderr, "[TRACE] SetupLibraryImports - iterating %u records\n", import_count); fflush(stderr);
  fprintf(stderr, "[TRACE] SetupLibraryImports - memory()=%p virtual_membase=%p\n",
          (void*)memory(), (void*)memory()->virtual_membase()); fflush(stderr);
  for (uint32_t i = 0; i < import_count; i++) {
    uint32_t record_addr = library->import_table[i];
    if (i < 5 || i % 100 == 0) {
      fprintf(stderr, "[TRACE]   import[%u/%u] addr=0x%08X\n", i, import_count, record_addr); fflush(stderr);
    }
    if (!record_addr) continue;

    if (i < 3) {
      auto* host_ptr = memory()->TranslateVirtual(record_addr);
      fprintf(stderr, "[TRACE]   translate(0x%08X) -> host=%p\n", record_addr, (void*)host_ptr); fflush(stderr);
    }
    auto record_slot =
        memory()->TranslateVirtual<rex::be<uint32_t>*>(record_addr);
    if (i < 3) { fprintf(stderr, "[TRACE]   reading record_slot=%p\n", (void*)record_slot); fflush(stderr); }
    uint32_t record_value = *record_slot;
    if (i < 3) { fprintf(stderr, "[TRACE]   record_value=0x%08X\n", record_value); fflush(stderr); }

    uint16_t record_type = (record_value & 0xFF000000) >> 24;
    uint16_t ordinal = record_value & 0xFFFF;

    if (i < 3) { fprintf(stderr, "[TRACE]   type=%u ordinal=%u\n", record_type, ordinal); fflush(stderr); }
    ...
    if (record_type == 0) {
      ...
      if (export_resolver) {
        if (i < 3) { fprintf(stderr, "[TRACE]   GetExportByOrdinal('%.*s', %u) resolver=%p\n", (int)base_name.size(), base_name.data(), ordinal, (void*)export_resolver); fflush(stderr); }

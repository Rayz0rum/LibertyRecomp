...
bool Processor::InitializeFunctionTable(uint32_t code_base, uint32_t code_size,
  ...
  // If memory already has the table (e.g., Liberty pre-populated it during
  // InitializeFromRexGlue), adopt it instead of failing.
  if (memory_->HasFunctionTable()) {
    REXLOG_INFO("Memory function table already initialized, adopting existing table");
  } else if (!memory_->InitializeFunctionTable(code_base, code_size, image_base, image_size)) {

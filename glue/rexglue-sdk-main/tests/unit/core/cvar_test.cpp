...
REXCVAR_DEFINE_INT32(test_ranged_flag, 5, "Test", "Test ranged flag")
    .range(1, 10);
...
TEST_CASE("cvar registry stores flag metadata", "[cvar]") {
    ...
    SECTION("Flags are registered") {
        ...
        for (const auto& name : flags) {
            if (name == "test_bool_flag") found_bool = true;
            if (name == "test_int32_flag") found_int32 = true;
            if (name == "test_string_flag") found_string = true;
        }
...
TEST_CASE("cvar ListFlagsByLifecycle", "[cvar]") {
    ...
    bool found_init = std::find(init_only.begin(), init_only.end(), "test_init_only_flag") != init_only.end();
    bool found_restart = std::find(restart.begin(), restart.end(), "test_restart_flag") != restart.end();
...
TEST_CASE("cvar reset and diff utilities", "[cvar]") {
    ...
    SECTION("ListModifiedFlags returns changed flags") {
        ...
        bool found_int = std::find(modified.begin(), modified.end(), "test_int32_flag") != modified.end();
...
REXCVAR_DEFINE_BOOL(test_debug_flag, false, "Test", "Debug-only flag")
    .debug_only();

 ...
 public:
  ...
  uint32_t image_size() const override {
    ...
    for (uint32_t i = 0; i < xex_security_info()->page_descriptor_count; i++) {
      ...
      desc.value =
          rex::byte_swap(xex_security_info()->page_descriptors[i].value);
  ...
  uint32_t export_table_address() const override {
    return xex_security_info()->export_table;
  }
  ...
  uint32_t exception_directory_address() const override { return base_address_ + exception_dir_rva_; }

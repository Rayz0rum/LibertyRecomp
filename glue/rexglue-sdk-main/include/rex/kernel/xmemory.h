...
#include <rex/runtime/guest/context.h>  // PPCFunc type (minimal header)
...
class BaseHeap {
 public:
  ...
  // range.
  rex::memory::PageAccess QueryRangeAccess(uint32_t low_address,
 ...
 protected:
  ...
  void Initialize(memory::Memory* memory, uint8_t* membase, HeapType heap_type,
...
class Memory {
 public:
  ...
  inline BaseHeap* LookupHeap(uint32_t address) {
    return const_cast<BaseHeap*>(
        const_cast<const memory::Memory*>(this)->LookupHeap(address));
  }
 ...
 private:
  ...
  rex::memory::FileMappingHandle mapping_ =
      rex::memory::kFileMappingHandleInvalid;
